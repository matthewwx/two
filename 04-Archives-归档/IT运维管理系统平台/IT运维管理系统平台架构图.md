...
> 📐 架构图在 Obsidian 中打开即可渲染为图形。提供三种格式：Mermaid（文本渲染）、Excalidraw（手绘风格）、Draw.io（专业图表）。

## 🎨 Excalidraw 版架构图

![[IT运维管理系统平台架构图.excalidraw]]
...
![[IT运维管理系统平台五层架构图.drawio]]

> 在 Obsidian 中安装 Draw.io 插件后可编辑。五层结构：用户入口层 → 展示层 → 管理操作层 → 数据采集层 → 数据存储层 → 被管理设备
